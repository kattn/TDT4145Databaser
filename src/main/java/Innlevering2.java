...
public class Innlevering2{
    ...
    private static boolean userStory4() {
        ...
        try {
            ...
            int differanseSpesifikk = Integer.valueOf(statement.executeQuery(sql).toString()) - Integer.valueOf(statement.executeQuery(sql).toString());
            System.out.println("Differnse mellom spesifikt mål og beste = " + differanseSpesifikk);

            int differanseAktiv = Integer.valueOf(statement.executeQuery(sql2).toString()) - Integer.valueOf(statement.executeQuery(sql2).toString());
